#!/usr/bin/env python3
"""
Generate pre-test and post-test PDFs.
Supports agency: "Attentive" (default) or "Abode".
"""
...
def create_name_date_overlay(staff_name, date_str, pw, ph, agency="Attentive"):
    """Overlay name, date, and agency name in the white space between grey and blue bars."""
    buf = BytesIO()
    c = canvas.Canvas(buf, pagesize=(pw, ph))

    agency_full = "Attentive Care Service Coordination" if agency == "Attentive" \
                  else "Abode Care Service Coordination"

    # White space between grey header bar and blue "5.0" bar
    # Grey bar bottom ≈ ph - 60, Blue bar top ≈ ph - 100
    y_pos = ph - 78  # centered in the white gap
...
def fill_pdf(field_values, staff_name, date_str, output_path, agency="Attentive"):
    ...
    overlay_bytes = create_name_date_overlay(staff_name, date_str, pw, ph, agency)
...
def generate_tests(staff_name, pre_date, post_date, output_dir, agency="Attentive"):
    ...
    fill_pdf(post_fields, staff_name, post_date, post_path, agency)

    pre_fields = generate_pretest_fields()
    pre_path = os.path.join(output_dir, f"{safe_name}_Pre_Test_{pre_date.replace('/', '-')}.pdf")
    fill_pdf(pre_fields, staff_name, pre_date, pre_path, agency)


if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python3 generate_tests.py <staff_name> <pre_date> <post_date> [output_dir] [agency]")
        sys.exit(1)

    generate_tests(
        sys.argv[1], sys.argv[2], sys.argv[3],
        sys.argv[4] if len(sys.argv) > 4 else "./output",
        sys.argv[5] if len(sys.argv) > 5 else "Attentive"
    )
